...
const CPUCoolingSimulation = () => {
  ...
  const drawMainSystem = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    if (tempRatio > 0.1) {
      ...
      for (let i = 0; i < 5; i++) {
        const gy = centerY + 90 + i * 40 - genOffset;
        if (gy < centerY + 250) {
          ctx.beginPath();
    ...
    ctx.fillText('Udara Panas', centerX - 200, fanY + 60);
    ctx.fillText('Keluar ↑', centerX - 200, fanY + 90);
    ctx.shadowBlur = 0;
    ...
    // CPU label - RIGHT with pointer
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 24px sans-serif';
    ctx.textAlign = 'left';
    ctx.shadowBlur = 20;
    ctx.shadowColor = '#ffffff';
    ctx.fillText('CPU CHIP', centerX + 180, centerY + 60);
    ctx.shadowBlur = 0;
    ...
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(centerX + 170, centerY + 60);
    ctx.lineTo(centerX + 80, centerY + 30);
    ctx.stroke();
    drawLeftArrow(ctx, centerX + 80, centerY + 30, centerX + 70, centerY + 25);
    ...
    ctx.fillText(`${temp.toFixed(1)}°C`, centerX, centerY + 12);
    ctx.shadowBlur = 0;
  ...
  const drawCPU = (ctx: CanvasRenderingContext2D, x: number, y: number) => {
    ...
    const hue = 120 - tempRatio * 120;
    const cpuGrad = ctx.createLinearGradient(x - size/2, y - size/2, x + size/2, y + size/2);
    ...
    for (let i = 1; i < 5; i++) {
      ctx.beginPath();
      ctx.moveTo(x - size/2 + (size/5) * i, y - size/2);
      ctx.lineTo(x - size/2 + (size/5) * i, y + size/2);
      ctx.stroke();
      
      ctx.beginPath();
      ctx.moveTo(x - size/2, y - size/2 + (size/5) * i);
      ctx.lineTo(x + size/2, y - size/2 + (size/5) * i);
      ctx.stroke();
    }
    ...
    for (let i = 0; i < 10; i++) {
      // Bottom pins
      ctx.fillRect(x - 50 + i * 11, y + size/2, 8, 8);
      // Top pins
      ctx.fillRect(x - 50 + i * 11, y - size/2 - 8, 8, 8);
      // Left pins
      ctx.fillRect(x - size/2 - 8, y - 50 + i * 11, 8, 8);
      // Right pins
      ctx.fillRect(x + size/2, y - 50 + i * 11, 8, 8);
    }
  };
          ...
          <div className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur rounded-2xl p-8 shadow-xl border border-slate-700/50">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              <div className="bg-gradient-to-br from-red-500/20 to-orange-600/20 backdrop-blur rounded-2xl p-6 border border-red-500/30 shadow-xl">
                <p className="text-slate-300 text-sm mb-3 font-semibold">Suhu Awal</p>
              ...
              <div className="bg-gradient-to-br from-blue-500/20 to-cyan-600/20 backdrop-blur rounded-2xl p-6 border border-blue-500/30 shadow-xl">
                ...
                <p className="text-4xl font-bold text-blue-400">T_amb</p>
                <p className="text-3xl font-bold text-blue-300 mt-2">{Tambient}°C</p>
              </div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 via-pink-500/10 to-rose-500/10 backdrop-blur rounded-2xl p-8 border border-purple-500/30 shadow-xl">
              ...
              <div className="space-y-3 text-slate-300">
                <div className="flex items-start gap-3">
                  <span className="text-2xl font-bold text-emerald-400 min-w-[60px]">T(t)</span>
                  <span className="text-base">= Suhu CPU pada waktu t (detik)</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-2xl font-bold text-cyan-400 min-w-[60px]">T_amb</span>
                  <span className="text-base">= Suhu udara sekitar (25°C)</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-2xl font-bold text-red-400 min-w-[60px]">T₀</span>
                  <span className="text-base">= Suhu awal CPU (80°C)</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-2xl font-bold text-yellow-400 min-w-[60px]">k</span>
                  <span className="text-base">= Konstanta pendinginan (0.15) - dipengaruhi oleh efisiensi airflow dan heatsink</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-2xl font-bold text-purple-400 min-w-[60px]">t</span>
                  <span className="text-base">= Waktu dalam detik</span>
                </div>
              </div>
            </div>
          </div>
